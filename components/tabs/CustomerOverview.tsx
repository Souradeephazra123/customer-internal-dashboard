...
// import { CSM_LIST } from "@/lib/constants";
...
//     () =>
//       Math.round(
//         activeCustomers.reduce((a, c) => a + c.nps, 0) /
...
//         <MetricCard
//           label="Total MRR"
//           value={`$${Math.round(totalMrr / 1000)}k`}
//           sub={`${activeCustomers.length} active accounts`}
//         />
//         <MetricCard label="Healthy" value={healthy} sub="customers" valueColor="#3B6D11" />
//         <MetricCard label="At Risk" value={atRisk} sub="need attention" valueColor="#BA7517" />
//         <MetricCard label="Churned" value={churned} sub="this year" valueColor="#A32D2D" />
//         <MetricCard label="Avg NPS" value={avgNps} sub="active accounts" />
//       </div>
...
//         </FilterSelect>
//         <FilterSelect value={csmFilter} onChange={(e) => setCsmFilter(e.target.value)}>
...
//           <thead>
//             <tr>
//               {["Customer", "Status", "Tier", "MRR", "CSM", "Location", "NPS", "Churn Risk"].map(
//                 (h) => (
//                   <th
//                     key={h}
...
//                   {fmtMrr(c.mrr)}
//                 </td>
//                 <td className="px-2.5 py-2 border-b border-gray-100">{c.csm}</td>
//                 <td className="px-2.5 py-2 border-b border-gray-100">
//                   {c.city}, {c.state}
//                 </td>
//                 <td className="px-2.5 py-2 border-b border-gray-100">{c.nps}</td>
//                 <td className="px-2.5 py-2 border-b border-gray-100">
...
// src/components/tabs/CustomerOverview.tsx
"use client";

import { useState, useMemo } from "react";
import { CUSTOMERS, getCustomerTenureMonths } from "@/data/customer";
import { MetricCard } from "@/components/ui/MetricCard";
...
export function CustomerOverview() {
  ...
  return (
    <>
      {/* KPI row — only real derived metrics, no fabricated NPS/churnRisk */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5 mb-5">
        <MetricCard
          label="Total MRR"
      ...
      {/* Table — every column maps to a real CSV field */}
      <div className="bg-white border border-gray-200 rounded-lg p-3.5 overflow-x-auto">
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr>
      ...
      {/* US Map — now uses real lat/lng from CSV */}
      <USMap />
    </>
  );
}